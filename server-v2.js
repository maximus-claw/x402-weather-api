...
// Map human network names to CAIP-2 IDs for x402 v2
const NETWORK_MAP = {
  "base-mainnet": "eip155:8453",
  "base-sepolia": "eip155:84532",
};
const CAIP_NETWORK = NETWORK_MAP[NETWORK] || NETWORK;
...
// Route configurations with discovery metadata for Bazaar
const x402Routes = {
  "GET /predict/all": {
    accepts: {
      scheme: "exact",
            ...
            cities: {
              NYC: {
                ...
                model: {
                   mu: 48.2,
                   sigma: 2.1,
                   confidence_intervals: { "95%": { low: 44.1, high: 52.3 } }
                }
              }
            }
          }
...
// Add individual city routes
for (const city of Object.keys(STATIONS)) {
  x402Routes[`GET /predict/${city}`] = {
    accepts: {
      scheme: "exact",
      price: "$0.01",
      network: CAIP_NETWORK,
        ...
        output: {
          example: {
            ...
            prediction: {
              mu: 72.5,
              sigma: 3.1,
              confidence_intervals: { "80%": { low: 68.5, high: 76.5 } }
            }
          }
        }
      })
...
app.listen(PORT, () => {
  console.log(`\n⚡ x402 Weather Prediction API v2.0 running on port ${PORT} [x402-v2-SDK]`);
  console.log(`   Network: ${NETWORK} (${CAIP_NETWORK})`);
